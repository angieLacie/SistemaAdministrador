...
const DetalleLicencia = () => {
  ...
  const [ocsProveedor, setOcsProveedor] = useState([]); // múltiples OC proveedor
  ...
  const cargarTodo = async () => {
    try {
      ...
      // Cargar OCs proveedor por oficina
      ...
      // Cargar OC Clientes con HES y Facturación
  ...
  const openOcModal = (oc = null) => {
    ...
    if (oc) {
      ...
      setFormData({
        numeroOc: '', numeroCO: '', idOrdenCO: null,
        solpedCompra: '', importeUsd: '', importePen: '',
  ...
  const crearNuevaCO = async () => {
    try {
      await ordenCOService.crear({
        ...
        idEmpresa:   oficina.idEmpresaUsuario,
        descripcion: '',
      });
  ...
  const saveOc = async () => {
    try {
      ...
      if (editItem) {
        await ordenesCompraService.editar(editItem.idOc, payload);
        toast.success('OC Proveedor actualizada correctamente');
      } else {
        await ordenesCompraService.crear({ ...payload, idEmpresa: oficina.idEmpresaUsuario ?? 1 });
        toast.success('OC Proveedor registrada correctamente');
      }
  ...
  const deleteOc = async (idOc) => {
    try {
      await ordenesCompraService.eliminar(idOc);
  ...
  const saveHes = async () => {
    try {
      ...
      if (editItem) {
        await hesService.editar(editItem.idHes, payload);
        toast.success('HES actualizada correctamente');
      } else {
        await hesService.crear(payload);
  ...
  const deleteHes = async (idHes) => {
    try {
      await hesService.eliminar(idHes);
  ...
  const openOcClienteModal = (item = null) => {
    ...
    setFormData(item ? {
      ...
      empresaFacturable: item.empresaFacturable  ?? '',
  ...
  const saveHesCliente = async () => {
    try {
      setSaving(true);
      const payload = {
        idOcCliente: parentItem.idOcCliente,
        numeroHes:   formData.numeroHes,
        descripcion: formData.descripcion || null,
        importeUsd:  parseFloat(formData.importeUsd) || null,
        importePen:  parseFloat(formData.importePen) || null,
      ...
      if (editItem) {
        await hesClienteService.editar(editItem.idHesCliente, payload);
        toast.success('HES Cliente actualizada correctamente');
      } else {
        await hesClienteService.crear(payload);
  ...
  const deleteHesCliente = async (idHesCliente) => {
    try {
      await hesClienteService.eliminar(idHesCliente);
  ...
  const saveFact = async () => {
    try {
      ...
      const payload = {
        idOcCliente:   parentItem.idOcCliente,
        idHesCliente:  parentItem.idHesCliente,
      ...
      if (editItem) {
        await facturacionService.editar(editItem.idFacturacion, payload);
  ...
  const deleteFact = async (idFact) => {
    try {
      await facturacionService.eliminar(idFact);
  ...
  return (
    <div className="content-wrapper">
      ...
      <div className="main-content">
        <Button variant="outline-secondary" size="sm" className="mb-3" onClick={() => navigate('/licencias')}>
          <FaArrowLeft size={11} className="me-1" /> Volver a licencias
        </Button>
        ...
        <div className="card mb-3" style={{ border: '1px solid #fca5a5' }}>
          <div className="card-header d-flex align-items-center justify-content-between py-2"
            ...
            <span className="fw-semibold small text-uppercase" style={{ color: '#991b1b', letterSpacing: '.06em' }}>
              OC Proveedor — compra de licencia
            </span>
          ...
          <div className="card-body">
            {ocsProveedor.length === 0 ? (
              <div className="text-center py-3">
                <p className="text-muted small mb-2">No hay OC de proveedor registradas.</p>
              </div>
            ) : (
              ocsProveedor.map((oc, i) => (
                <div key={oc.idOc} className="mb-3 p-3 rounded"
                  ...
                  <div className="d-flex justify-content-between align-items-center mb-2">
                    ...
                    <div className="d-flex gap-1">
                      ...
                      <Button size="sm" variant="outline-danger" onClick={() => deleteOc(oc.idOc)}>
                        <FaTrash size={10} />
                      </Button>
                    </div>
                  </div>
                  ...
                  <Row className="g-2 mb-3">
                    ...
                    <Col md={3}><div className="text-muted" style={{ fontSize: 11 }}>Solped de compra</div><div className="font-monospace">{oc.solpedCompra ?? '—'}</div></Col>
                  ...
                  <div style={{ borderTop: '1px solid #fca5a5', paddingTop: 10 }}>
                    <div className="d-flex align-items-center justify-content-between mb-2">
                      <small className="fw-semibold text-uppercase" style={{ color: '#991b1b', fontSize: 10 }}>
                        HES — hojas de entrada de servicios
                      </small>
                    ...
                    {oc.hes?.length === 0 ? (
                      <p className="text-muted small">No hay HES registradas.</p>
                    ) : (
                            ...
                            <tr key={h.idHes}>
                              ...
                              <td>
                                <div className="d-flex gap-1">
                                  ...
                                  <Button size="sm" variant="outline-danger" onClick={() => deleteHes(h.idHes)}><FaTrash size={10} /></Button>
                                </div>
                              </td>
                            </tr>
                          ))}
        ...
        <div className="card mb-3" style={{ border: '1px solid #d1d5db' }}>
          ...
          <div className="card-body">
            {ocClientes.length === 0 ? (
              <p className="text-muted small">No hay OC de cliente registrada para esta oficina.</p>
            ) : (
              ocClientes.map((occ, i) => (
                <div key={occ.idOcCliente} className="mb-3 p-3 rounded"
                  ...
                  <div style={{ borderTop: '1px solid #e5e7eb', paddingTop: 10 }}>
                    ...
                    {occ.hesClientes?.length === 0 ? (
                      ...
                      occ.hesClientes?.map(h => (
                        <div key={h.idHesCliente} className="mb-2 p-2 rounded"
                          style={{ background: '#fff', border: '0.5px solid #e5e7eb' }}>

                          <div className="d-flex align-items-center justify-content-between mb-2">
                            <div className="d-flex align-items-center gap-2">
                              <span className="fw-semibold font-monospace small" style={{ color: '#374151' }}>{h.numeroHes}</span>
                            ...
                            <div className="d-flex gap-1">
                              ...
                              <Button size="sm" variant="outline-danger" onClick={() => deleteHesCliente(h.idHesCliente)}><FaTrash size={10} /></Button>
                            </div>
                          </div>
                                    ...
                                    <tr key={f.idFacturacion}>
                                      ...
                                      <td>
                                        <div className="d-flex gap-1">
                                          ...
                                          <Button size="sm" variant="outline-danger" onClick={() => deleteFact(f.idFacturacion)}><FaTrash size={9} /></Button>
                                        </div>
                                      </td>
                                    </tr>
                                  ))}
        ...
        <ModalBody>
          <Row className="g-3">
            <Col md={6}>
              ...
              <Form.Control
                name="numeroCO"
                value={formData.numeroCO ?? ''}
                onChange={(e) => { handleChange(e); setValidacionCO(null); }}
                onBlur={validarOrdenCO}
                placeholder="Ej: 10000022283"
                isValid={validacionCO === 'ok'}
                isInvalid={validacionCO === 'notfound'}
              />
            ...
            <Col md={6}>
              <Form.Label className="small fw-semibold">N° Orden de Compra *</Form.Label>
              <Form.Control name="numeroOc" value={formData.numeroOc ?? ''} onChange={handleChange} placeholder="Ej: 4500200005"/>
            </Col>
            <Col md={6}><Form.Label className="small fw-semibold">Solped de compra</Form.Label>
              <Form.Control name="solpedCompra" value={formData.solpedCompra ?? ''} onChange={handleChange} placeholder="Ej: 10031836"/>
            </Col>
            <Col md={6}><Form.Label className="small fw-semibold">Importe USD</Form.Label>
              <Form.Control type="number" name="importeUsd" value={formData.importeUsd ?? ''} onChange={handleChange} placeholder="0.00"/>
            </Col>
            <Col md={6}><Form.Label className="small fw-semibold">Importe PEN</Form.Label>
              <Form.Control type="number" name="importePen" value={formData.importePen ?? ''} onChange={handleChange} placeholder="0.00"/>
            </Col>
            <Col md={6}><Form.Label className="small fw-semibold">Estado</Form.Label>
            ...
            <Col md={12}><Form.Label className="small fw-semibold">Descripción</Form.Label>
              <Form.Control name="descripcion" value={formData.descripcion ?? ''} onChange={handleChange} placeholder="Descripción de la OC"/>
            </Col>
          </Row>
        </ModalBody>
        <ModalFooter>
        ...
        <ModalBody>
          <Row className="g-3">
            <Col md={6}><Form.Label className="small fw-semibold">N° HES *</Form.Label>
              <Form.Control name="numeroHes" value={formData.numeroHes ?? ''} onChange={handleChange} placeholder="Ej: HES-001"/>
            </Col>
            <Col md={6}><Form.Label className="small fw-semibold">Fecha</Form.Label>
              <Form.Control type="date" name="fechaHes" value={formData.fechaHes ?? ''} onChange={handleChange}/>
            </Col>
            <Col md={6}><Form.Label className="small fw-semibold">Importe USD</Form.Label>
              <Form.Control type="number" name="importeUsd" value={formData.importeUsd ?? ''} onChange={handleChange} placeholder="0.00"/>
            </Col>
            ...
            <Col md={12}><Form.Label className="small fw-semibold">Descripción</Form.Label>
              <Form.Control name="descripcion" value={formData.descripcion ?? ''} onChange={handleChange} placeholder="Descripción"/>
            </Col>
          </Row>
        </ModalBody>
        <ModalFooter>
        ...
        <ModalBody>
          <Row className="g-3">
            <Col md={6}><Form.Label className="small fw-semibold">Cod. SAP cliente</Form.Label>
              <Form.Control name="codSapCliente" value={formData.codSapCliente ?? ''} onChange={handleChange} placeholder="Ej: 4500070668"/>
            </Col>
            <Col md={6}><Form.Label className="small fw-semibold">Empresa facturable</Form.Label>
              <Form.Control name="empresaFacturable" value={formData.empresaFacturable ?? ''} onChange={handleChange} placeholder="Ej: R010 - El S.A."/>
            </Col>
            <Col md={6}><Form.Label className="small fw-semibold">Importe ref. PEN</Form.Label>
              <Form.Control type="number" name="importeRefPen" value={formData.importeRefPen ?? ''} onChange={handleChange} placeholder="0.00"/>
            </Col>
            <Col md={6}><Form.Label className="small fw-semibold">Importe ref. USD</Form.Label>
              <Form.Control type="number" name="importeRefUsd" value={formData.importeRefUsd ?? ''} onChange={handleChange} placeholder="0.00"/>
            </Col>
          </Row>
        </ModalBody>
        <ModalFooter>
        ...
        <ModalBody>
          <Row className="g-3">
            ...
            <Col md={6}><Form.Label className="small fw-semibold">Año/Mes facturación</Form.Label>
              <Form.Control name="anioMesFac" value={formData.anioMesFac ?? ''} onChange={handleChange} placeholder="YYYYMM"/>
            </Col>
            <Col md={6}><Form.Label className="small fw-semibold">Nro. Factura SAP</Form.Label>
              <Form.Control name="nroFacturaSap" value={formData.nroFacturaSap ?? ''} onChange={handleChange} placeholder="Ej: 91431893"/>
            </Col>
            <Col md={6}><Form.Label className="small fw-semibold">Monto fact. PEN</Form.Label>
              <Form.Control type="number" name="montoFactPen" value={formData.montoFactPen ?? ''} onChange={handleChange} placeholder="0.00"/>
            </Col>
            <Col md={6}><Form.Label className="small fw-semibold">Monto fact. USD</Form.Label>
              <Form.Control type="number" name="montoFactUsd" value={formData.montoFactUsd ?? ''} onChange={handleChange} placeholder="0.00"/>
            </Col>
          </Row>
        </ModalBody>
        <ModalFooter>
